...
| `test/events/stats.ts` | `countCharsForWordIndex` đếm theo phím (`countKeysAsChars`); phần trăm của `getAccuracy` lấy từ cờ đúng/sai của từng phím lúc gõ, nên lỗi đã sửa vẫn bị trừ; phím cách chốt một từ còn thiếu thì tính mỗi phím còn thiếu (`unpaidKeys`) là một lần sai, thay cho cờ sai của chính phím cách | WPM theo phím đáng tốn, xem `beartype/scoring.ts`; phím đúng hay sai chấm bằng `isWrongKey`, nên chữ đang dựng dấu không bị tính sai |
